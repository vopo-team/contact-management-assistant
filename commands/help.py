...
def help() -> str:
    commands = [
        ...
        {"command": "all", "params": "", "desc": "- Show all contacts"},
    ...
    for cmd in commands:
        tag = (
            InlineFormatter(f"{cmd['command']:<12}")
            .color(cmd.get("tag_color", "yellow"))
            .format()
        )
        params = (
            InlineFormatter(f"{cmd['params']:<30}")
            .color(cmd.get("param_color", "green"))
            .format()
        )
        desc = (
            InlineFormatter(f"{cmd['desc']}")
            .color(cmd.get("desc_color", "white"))
            .format()
        )
